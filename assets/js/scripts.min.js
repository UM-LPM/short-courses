...
!function (c) {
  c.fn.simpleJekyllSearch = function (e) {
    var l = c.extend({
      jsonFile: "/course-data.json",
      jsonFormat: "title,keywords,author,url,description",
    ...
    function normalizeText(text) {
      return text.replace(/š/g, "s").replace(/č/g, "c").replace(/ž/g, "z")
    ...
    function displayResults(matches) {
      ...
      if (matches.length) {
        c.each(matches, function (index, item) {
          if (index < l.limit) {
            var resultHtml = l.template;
            c.each(a, function (_, field) {
              var fieldValue = item[field] || "";
              resultHtml = resultHtml.replace(new RegExp("{" + field + "}", "g"), fieldValue);
            });
            i.append(c(resultHtml));
          }
        });
      } else {
    ...
    l.jsonFile.length && i.length && c.ajax({
      ...
      success: function (e) {
        o = e;
        s.keyup(function () {
          var inputVal = c(this).val();
          if (inputVal.length) {
            var normalizedInput = normalizeText(inputVal).toLowerCase();
            var matches = [];

            c.each(o, function (_, item) {
              for (var i = 0; i < a.length; i++) {
                var field = item[a[i]];
                if (field) {
                  var normalizedField = normalizeText(field.toString().toLowerCase());
                  var originalField = field.toString().toLowerCase();
                  if (normalizedField.indexOf(normalizedInput) !== -1 || originalField.indexOf(inputVal.toLowerCase()) !== -1) {
                    matches.push(item);
                    break;
                  }
                }
...
!function (e, t) {
  ...
  function fetchAndDisplayCourses() {
    fetch('course-data.json')
      ...
      .then(data => {
        ...
        data.forEach(course => {
          const courseDate = course.unparsedExecutionStartDate
            ? new Date(course.unparsedExecutionStartDate)
            : null;

          const publishDate = course.publishDate
            ? new Date(course.publishDate)
            : null;

          const removalDate = course.removalDate && course.removalDate.trim() !== ""
            ? new Date(course.removalDate)
            : null;

          const isVisibleNow = publishDate && now >= publishDate && (!removalDate || now < removalDate);

          const isFutureExecution = courseDate && courseDate >= now;

          if (isVisibleNow && isFutureExecution) {
        ...
        coursesWithDate.sort((a, b) =>
          new Date(a.unparsedExecutionStartDate) - new Date(b.unparsedExecutionStartDate)
        );

        const finalCourses = [...coursesWithDate];
  ...
  function createCourseElement(course) {
    ...
    const keywordsArray = course.keywords ? course.keywords.split(', ') : [];

    const authors = course.author;
    const startDate = course.executionStartDate;
    const endDate = course.executionEndDate;
    const datesString = (course.executionStartDate && course.executionEndDate) ? `${startDate} - ${endDate}` : null;
    const imageUrl = course.url ? `${course.image}/512${course.url}.png` : `/assets/img/placeholder.png`;

    const keywordTagsHtml = keywordsArray.map(tag => `<a href="/tags/#${tag.trim().toLowerCase().replace(/ /g, '-')}" class="tag">#${tag}</a>`).join(' ');

    article.innerHTML = `
          <span class="category"><span>${course.title}</span></span>
          <span class="category"><span style="color: darkred;">${course.filled === "true" ? 'Zaseden termin' : ''}</span></span>
          <div class="box-body">
            <a class="cover" href="${course.url}">
              <img src="${imageUrl}" width="100%" class="preload">
              <noscript><img src="/assets/img/off.jpg" width="100%"></noscript>
              <div class="read-icon">
            ...
            <div class="box-info">
              <a class="course-link" href="${course.url}">
                ...
                <h3 class="course-dates" style="margin-top:0;">Predvidena starost: ${course.intendedFor}</h3>
                <h3 class="course-author" style="margin-top:0;">${authors}</h3>
                <p class="description">${shortenDescription(course.description)}</p>
              </a>
