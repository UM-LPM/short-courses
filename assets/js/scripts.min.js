...
!function (c) {
    c.fn.simpleJekyllSearch = function (e) {
        var l = c.extend({
            ...
            jsonFormat: "title,keywords,lecturersStrSl,acronym,descriptionSl,klasiusCodes,facultyStr",
            template: '<li><article><a href="/{acronym}"><span class="entry-faculty"> {facultyStr}</span>\ {title} \
<span class="entry-author">{lecturersStrSl}</span>\
<span class="entry-keywords">{keywords}</span>\
<span class="entry-klasius">KLASIUS-P-16 ({klasiusCodes})</span>\
</a></article></li>',
            searchResults: ".search-results",
            limit: "10",
            noResults: "<p>Žal, nismo našli rezultatov :(</p>"
        }, e),
                        ...
                        c.each(o, function (_, item) {
                            for (var i = 0; i < a.length; i++) {
                                ...
                                if (field) {
                                    ...
                                    if (normalizedField.indexOf(normalizedInput) !== -1 ||
                                        originalField.indexOf(inputVal.toLowerCase()) !== -1) {
                                        matches.push(item);
                                        break;
                                    }
                                }
...
!function (e, t) {
    ...
    function createCourseElement(course) {
        ...
        const keywordTagsHtml = keywordsArray
            .map(
                tag =>
                    `<a href="/tags/#${tag.toLowerCase().replace(/ /g, "-")}" class="tag">#${tag}</a>`
            )
            .join(" ");

        article.innerHTML = `
    <span class="category"><span>${title}</span></span>
    <div class="box-body">
      ...
      <div class="box-info">
        <a class="course-link" href="${course.url}">
          ...
          <p class="description">${shortenDescription(description)}</p>
        </a>
...
function setLangCookie(e) { var t = new Date(Date.now() + 31536e6).toUTCString(); document.cookie = "pll_language=" + e + "; expires=" + t + "; path=/; SameSite=Lax" } function updatePlaceholders(e) { document.querySelectorAll("[data-placeholder-sl]").forEach(function (t) { t.placeholder = "en" === e ? t.getAttribute("data-placeholder-en") || t.getAttribute("data-placeholder-sl") : t.getAttribute("data-placeholder-sl") }) } function toggleLang() { var e = document.documentElement, t = e.classList.contains("lang-en") ? "sl" : "en"; e.classList.remove("lang-sl", "lang-en"), e.classList.add("lang-" + t), localStorage.setItem("lang", t), setLangCookie(t), updatePlaceholders(t) } var _initLang = localStorage.getItem("lang") || "sl"; setLangCookie(_initLang), updatePlaceholders(_initLang);
